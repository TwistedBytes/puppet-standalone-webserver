class tbapache::mod::security (
  ...
  $allowed_http_versions = 'HTTP/0.9 HTTP/1.0 HTTP/1.1 HTTP/2',
  $content_types         = 'application/x-www-form-urlencoded|multipart/form-data|text/xml|application/xml|application/x-amf|application/json',
  ...
  file { "${modsec_dir}/security_crs.conf":
    ...
    content => template('tbapache/mod/security_crs.conf.erb'),
    require => File[$modsec_dir],
    notify  => Class['apache::service'],
  }

  tbapache::security::rule_link { $activated_rules: }
