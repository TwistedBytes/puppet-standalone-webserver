...
USER = 'root'
PASS = '<%= scope.lookupvar('mysql::server::root_password') %>'
...
class MysqlCheckHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            db = MySQLdb.connect(host='localhost', user=USER, passwd=PASS, db='mysql', port=int(MYSQLPORT), connect_timeout=1)
            ...
            if row == None:
                ...
                # assume slave
                if row['Slave_IO_Running'] != 'Yes' or row['Slave_SQL_Running'] != 'Yes' or row['Seconds_Behind_Master'] > int(MAX_LAG):
                    status = 503
